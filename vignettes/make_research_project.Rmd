...
To create a research project, follow these steps and use the `rUM` package. This will a new RStudio project that has an [`analysis.qmd`](#fig:analysis_quarto_rUM) Quarto file using the `tidyverse` and `conflicted` packages. For more advanced analysis, we can also create Quarto files which use the `tidymodels` package.
