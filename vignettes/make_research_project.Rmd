...
## Would you rather a `quarto`+`rUM` ?

Here's another way to get more `rUM` with [`quarto`](https://quarto.org/). If you already have already created a folder containing important files for your project:
...
```
PATH <- getwd()
make_quarto_project(PATH)
```

Instead of `analysis.Rmd` you will now see [`analysis.qmd`](#fig:analysis_quarto_rUM) with all of the other important `rUM` files.
