...
```
  ...
  * or navigating [here](https://raymondbalise.github.io/rUM/) or [here](https://github.com/RaymondBalise/rUM). 


Lastly, run this in the console of RStudio:
...
We would like to create a research project, so we use the `rUM` package. It creates a new RStudio project that has an `analysis.qmd` Quarto file using the `tidyverse` and `conflicted` packages. For more advanced analysis, we can also create Quarto files which use the `tidymodels` package.
...
* [`analysis.qmd`](#fig:analysis_quarto_rUM) is a Quarto boilerplate for writing your research project. It has a preconfigured YAML header; setup chunk; Introduction, Methods, Results, Conclusion, and Reference sections; and a code chunk to construct your bibliography using `knitr::write_bib()`.
