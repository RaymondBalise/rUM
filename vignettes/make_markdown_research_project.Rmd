...
To create a research project, follow these steps use the `rUM` package. This will initialize a new RStudio project that has an [`analysis.Rmd`](#fig:analysis_markdown) R markdown file using the `tidyverse` and `conflicted` packages. For more advanced analysis, we can also create R markdown files which use the `tidymodels` package.
