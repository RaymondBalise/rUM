...
#' 
#' @description This function makes an R project that includes an analysis.qmd 
...
#'    \code{.qmd} file called "analysis", a subdirectory for data, a template
...
#' @examples 
#' \dontrun{
#'   make_quarto_project(path = "~/test_project")
#' }

make_quarto_project <- function(path) {
  ...
  header <- c(
    ...
    '',
    '# Method',
    'Analyses were conducted with `r stringr::word(R.Version()$version.string, 1, 3)` with the `quarto` (`r packageVersion("quarto")`), `tidyverse` (`r packageVersion("tidyverse")`), `rUM` (`r packageVersion("rUM")`), `table1` (`r packageVersion("table1")`) packages used to preprocess and summarize data.[@R-base; @R-tidyverse; @tidyverse2019; @R-rUM; @R-table1]',
    '',
    '# Results',
    '',
    '# Conclusion',
  ...
  # write to index file
  writeLines(contents, con = file.path(path, "analysis.qmd"))
