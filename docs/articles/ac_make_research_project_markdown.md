...
papers using R. `rUM` will initialize a new RStudio project and a
Markdown file that includes the outline for a research paper. The
Markdown file comes preconfigured with a YAML header (don’t worry if you
don’t know what that means yet) with code chunks to load the `tidyverse`
and `conflicted` packages. Manuscript sections have been created for
...
1.  Modern version RStudio (v2022.07 or later) ships with R Markdown
...
[`analysis.Rmd`](#fig:analysis_Rmd_file) R Markdown file using the
...
3.  Specify the location of where your research project will be saved
    ...
    RStudio](save_markdown_project_here.png)
...
A new project directory has been created and it will be populated with
[these files](#fig:created_rUM_markdown_files):
...
- [`analysis.Rmd`](#fig:analysis_Rmd_file) is a Markdown template for
...
- Two text files, `packages.bib` and `references.bib`, which are used to
  ...
  [`analysis.Rmd`](#fig:analysis_Rmd_file) file for initial examples of
  how to add/use references.
...
pane](created_rUM_markdown_files.png)
...
1.  Navigate to File \> New File \> R Markdown
    ...
    ![Navigating the RStudio console to access R Markdown rUM
    headers](newfile_markdown.jpg)
    Navigating the RStudio console to access R Markdown rUM headers

2.  Select **From Template**

    ![RStudio menu window displaying to choose “html2 with rUM” to use
    the R Markdown headers](from_template.png)
    RStudio menu window displaying to choose “html2 with rUM” to use the
    R Markdown headers
...
this directory, you will find the `analysis.Rmd` R Markdown file. For
example, if you created an R Markdown file called
`wrangle_cytometry_data.Rmd` with the steps above, then your current
directory will now have a subdirectory called `wrangle_cytometry_data/`
which will contain the file `wrangle_cytometry_data.Rmd` and any
subsequent files from the knitting process (such as `.PDF`, `.html`, or
`.docx` files created by knitting the R Markdown document).
